...
export default function HostelType() {
  ...
  const [occupancies, setOccupancies] = useState<Record<string, string>>(() => {
    ...
    for (let i = 1; i <= 30; i++) {
      initial[i.toString()] = "0/6";
    }
    return initial;
  });

  const getStatus = (occ: string): Room["status"] => {
    if (occ === "0/6") return "empty";
    if (occ === "6/6") return "full";
    return "partial";
  };
  ...
  const handleRoomPress = (roomNumber: string) => {
    ...
    setOccupancies((prev) => {
      const current = prev[roomNumber] || "0/6";
      const [taken] = current.split("/").map(Number);
      const nextTaken = taken >= 6 ? 0 : taken + 1;
      return { ...prev, [roomNumber]: `${nextTaken}/6` };
    });
  };
    ...
    <ScrollView style={styles.body}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>
          Nyberg Hall - 6 students per room
        </Text>
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={[styles.floorRow, isSmallScreen && { gap: 12 }]}>
          {["A", "B", "C", "D"].map((floor) => (
            <Link
              key={floor}
              href={`/Regular/Nyberg/Floor${floor}` as Href}
              asChild
            >
              <Pressable
                style={{
      ...
      {/* Floor A Rooms */}
      <View style={styles.card}>
        <Text style={styles.sectionTitle}>Floor D- Rooms (1-30)</Text>
